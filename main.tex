\documentclass[
  ngerman,
  a4paper,
  12pt
]{report}

\usepackage[utf8]{inputenc}
\usepackage[english, german]{babel}
\usepackage[left=2.5cm, right=2.5cm]{geometry}
\usepackage{csquotes}
\usepackage{setspace}
\usepackage{caption}
\usepackage{xparse}
\usepackage[dvipsnames]{xcolor}
\usepackage{siunitx}
\usepackage{booktabs}

\usepackage{enumitem}
\setlist{itemsep=0pt}

\input{preamble/tikz.tex}
\input{preamble/minted.tex}
\input{preamble/commands.tex}

\usepackage[unicode]{hyperref}
\hypersetup{
  linkcolor = Red,
  filecolor = Magenta,
  urlcolor = Green,
  citecolor = Cyan
}
\urlstyle{same}

\usepackage{amsmath}

\usepackage[style = authoryear-ibid]{biblatex}
\addbibresource{preamble/main.bib}

\usepackage{graphicx}
\graphicspath{{./images}}

\usepackage[tikz]{mdframed}

\NewDocumentEnvironment{aquote}{m}
  {\begin{quote}}
  {\\\hspace*{16pt}--- #1\end{quote}}

\begin{document}
\pagenumbering{Roman}
\include{pages/titlepage.tex}
\onehalfspacing
\include{pages/erklärung.tex}
\include{pages/abstract.tex}
\tableofcontents
\listoffigures
\listoftables
\newpage
\pagenumbering{arabic}
\include{chapters/einleitung/einleitung.tex}
\include{chapters/first-nn/first-nn.tex}
\include{chapters/application/application.tex}
\printbibliography[title = Literaturverzeichnis]
\addcontentsline{toc}{chapter}{Literaturverzeichnis}
\appendix
\include{appendix/gd/gd.tex}
\end{document}