...
Die folgende Ausgabe ist gekürzt und zeigt nur die wichtigsten Werte:
\begin{pyconcode}
...
[{"index": 12,
  ...
  "dtype": numpy.float32,
  "quantization": (0.0, 0)}]
>>> # Weitere Attribute (dieselben wie für die Eingabedetails)
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\plotw]{first-nn/model-comparison.pdf}
...
$ # Führe das heruntergeladene Bash-Skript aus um die Modelle herunterzuladen
...
können wir einen weiteren Test durchführen,
...
print(f"TPU Inferenzgeschwindigkeit: {result:.4f} ms")
\end{pythoncode}
Wurden die vorigen Befehle ausgeführt, kann auch dieser Test einfach durchgeführt
werden:
\begin{consolecode}
...
Der Gründe für die langsamere Geschwindigkeit im ersten Test
...
eingesetzt (wie, dem \textit{Convolutional Neural Network} für Computervision
...
Werden die oben zu sehenden Benchmarks vier weitere Male durchgeführt, ergeben sich
Inferenzgeschwindigkeiten wie in \autoref{tab:benchmark-results}
zu sehen sind.
\begin{table}[h!]
  \centering
  \caption{Ein Vergleich der Inferenzgeschwindigkeiten, nachdem die
  Benchmarks viermal hintereinander durchgeführt wurden}
